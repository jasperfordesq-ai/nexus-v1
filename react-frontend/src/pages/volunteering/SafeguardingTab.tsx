...
export function SafeguardingTab() {
  ...
  const handleSubmitTraining = async () => {
    ...
    try {
      ...
      if (res.success) { trainingModal.onClose(); setTrainingForm({ training_type: 'children_first', training_name: '', provider: '', completed_at: '', expires_at: '' }); toastRef.current.success(tRef.current('safeguarding.training_added')); load(); }
      else { toastRef.current.error(tRef.current('safeguarding.training_failed')); }
    } catch (err) { logError('Failed to submit training record', err); toastRef.current.error(tRef.current('safeguarding.training_failed')); }
  ...
  return (
    <div className="space-y-4">
      ...
      {!error && !isLoading && subView === 'training' && (trainings.length === 0 ? (<EmptyState icon={<GraduationCap className="w-12 h-12" aria-hidden="true" />} title={t('safeguarding.no_training_title')} description={t('safeguarding.no_training_desc')} action={<Button className="bg-gradient-to-r from-rose-500 to-pink-600 text-white" onPress={trainingModal.onOpen}>{t('safeguarding.add_training')}</Button>} />) : (
        <motion.div variants={cV} initial="hidden" animate="visible" className="space-y-3">
          {trainings.map((tr) => (<motion.div key={tr.id} variants={iV}><GlassCard className="p-4"><div className="flex items-start justify-between gap-3"><div className="flex-1 min-w-0"><div className="flex items-center gap-2 mb-1"><span className="text-sm font-semibold text-theme-primary">{tr.training_name}</span><Chip size="sm" color={trainingStatusColor(tr.status)} variant="flat">{t(`safeguarding.status_${tr.status}`)}</Chip></div><p className="text-xs text-theme-muted">{t(`safeguarding.training_types.${tr.training_type}`)}{tr.provider && ` — ${tr.provider}`}</p><div className="flex items-center gap-3 mt-1 text-xs text-theme-subtle"><span className="flex items-center gap-1"><Calendar className="w-3 h-3" aria-hidden="true" />{t('safeguarding.completed')}: {new Date(tr.completed_at).toLocaleDateString()}</span>{tr.expires_at && (<span>{t('safeguarding.expires')}: {new Date(tr.expires_at).toLocaleDateString()}</span>)}</div></div></div></GlassCard></motion.div>))}
        </motion.div>))}
