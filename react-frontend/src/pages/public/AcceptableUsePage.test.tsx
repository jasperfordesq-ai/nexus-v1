...
describe('AcceptableUsePage', () => {
  ...
  it('renders spinner while loading', () => {
    ...
    expect(container.querySelector('[class*="spinner"]') || container.querySelector('div')).toBeTruthy();
  });

  it('renders custom document when available', () => {
    mockUseLegalDocument.mockReturnValue({
      document: { title: 'Custom AUP', content: '<p>Custom content</p>' },
      loading: false,
    });
    render(<AcceptableUsePage />);
    expect(screen.getByTestId('custom-legal-doc')).toBeInTheDocument();
    expect(screen.getByText('Custom AUP')).toBeInTheDocument();
  });
