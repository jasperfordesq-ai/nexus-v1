...
import { Mail, Save, Send, Shield, Globe } from 'lucide-react';
...
export function EmailSettings() {
    ...
    adminSettings.getEmailConfig()
      .then((res: ApiResponse<Record<string, unknown>>) => {
        ...
        if (data) {
          ...
          setFormData(prev => ({
            ...prev,
            provider: data.provider ?? 'platform_default',
            webhook_url: data.webhook_url ?? '',
            platform_default: data.platform_default ?? { provider: 'unknown' },
            // SendGrid (flatten nested response)
            sendgrid_api_key: sg?.api_key_masked ?? '',
            sendgrid_from_email: sg?.from_email ?? '',
            sendgrid_from_name: sg?.from_name ?? '',
            // Gmail API
            gmail_client_id: gmail?.client_id ?? '',
            gmail_client_secret: gmail?.client_secret_set ? '********' : '',
            gmail_refresh_token: gmail?.refresh_token_set ? '********' : '',
            gmail_sender_email: gmail?.sender_email ?? '',
            gmail_sender_name: gmail?.sender_name ?? '',
            // SMTP
            smtp_host: smtp?.host ?? '',
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      // Build payload with flat keys matching PHP endpoint expectations
  ...
  const provider = String(formData.provider || 'platform_default');
          ...
          <Card shadow="sm">
            ...
            <CardBody className="gap-4">
              <Input
                ...
                variant="bordered"
                description="Your SendGrid API key (stored encrypted)"
                value={String(formData.sendgrid_api_key || '')}
              ...
              <Input
                ...
                variant="bordered"
                value={String(formData.sendgrid_from_email || '')}
              ...
              <Input
                ...
                variant="bordered"
                value={String(formData.sendgrid_from_name || '')}
                onValueChange={(v) => updateField('sendgrid_from_name', v)}
              />
              {!!formData.webhook_url && (
                <div>
                  <p className="text-sm font-medium text-default-700 mb-1">Webhook URL</p>
                  <p className="text-sm text-default-500 bg-default-100 rounded-lg px-3 py-2 font-mono break-all">
                    {String(formData.webhook_url)}
                  </p>
                </div>
              )}
            </CardBody>
          ...
          <Card shadow="sm">
            ...
            <CardBody className="gap-4">
              <Input
                ...
                variant="bordered"
                value={String(formData.gmail_client_id || '')}
              ...
              <Input
                ...
                variant="bordered"
                value={String(formData.gmail_client_secret || '')}
              ...
              <Input
                ...
                type="password"
                variant="bordered"
                value={String(formData.gmail_refresh_token || '')}
              ...
              <Input
                ...
                placeholder="noreply@example.com"
                variant="bordered"
                value={String(formData.gmail_sender_email || '')}
              ...
              <Input
                ...
                placeholder="My Timebank"
                variant="bordered"
                value={String(formData.gmail_sender_name || '')}
          ...
          <Card shadow="sm">
            ...
            <CardBody className="gap-4">
              <Input
                ...
                variant="bordered"
                value={String(formData.smtp_host || '')}
              ...
              <Input
                ...
                variant="bordered"
                value={String(formData.smtp_user || '')}
              ...
              <Input
                ...
                type="password"
                variant="bordered"
                value={String(formData.smtp_password || '')}
                onValueChange={(v) => updateField('smtp_password', v)}
              />
              <Select
                label="Encryption"
                selectedKeys={[String(formData.smtp_encryption || 'tls')]}
                onSelectionChange={(keys) => {
                  const v = Array.from(keys)[0];
              ...
              <Input
                ...
                placeholder="noreply@example.com"
                variant="bordered"
                value={String(formData.smtp_from_email || '')}
              ...
              <Input
                ...
                placeholder="My Timebank"
                variant="bordered"
                value={String(formData.smtp_from_name || '')}
            ...
            <CardBody>
              <p className="text-sm text-default-500">
                ...
                <Chip size="sm" variant="flat" color="secondary">
                  {String((formData.platform_default as Record<string, unknown>)?.provider || 'Not configured')}
                </Chip>
              </p>
