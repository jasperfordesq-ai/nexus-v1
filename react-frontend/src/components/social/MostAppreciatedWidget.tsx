  ...
  return (
    <Card className={className}>
      <CardHeader className="flex items-center gap-2">
        ...
        <h3 className="font-semibold">{t('appreciations.most_title', 'Most appreciated')}</h3>
      </CardHeader>
      <CardBody className="space-y-2">
        {loading ? (
          ...
          <p className="text-sm text-[var(--text-muted)]">
            {t('appreciations.most_empty', 'No appreciations yet')}
          </p>
        ) : (
          rows.map((r, idx) => (
            <Link
            ...
            >
              ...
              <span className="flex-1 truncate text-sm">{r.name ?? t('common.someone', 'Someone')}</span>
