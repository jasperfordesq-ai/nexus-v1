...
import type { RegistrationQuestion } from '@/lib/event-registration-api';
import {
  validateRegistrationAnswers,
  visibleRegistrationAnswers,
  visibleRegistrationQuestions,
} from './event-registration-form-rules';
...
describe('event registration form rules', () => {
  it('matches every server visibility operator and evaluates cascading conditions in order', () => {
    const questions = [
      question('audience'),
    ...
    const answers = {
      ...
      tags: ['access'],
      equals: 'visible',
      unanswered: 'stale-hidden-answer',
  ...
  it('enforces required and min/max rules only for questions visible to the attendee', () => {
    const questions = [
      question('name', {
        is_required: true,
