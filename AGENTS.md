...
[HeroUI v2→v3 Migration Docs]|root: ./.heroui-docs/migration|This project is mid-migration from HeroUI v2 to v3 using incremental coexistence (Strategy A: npm aliases). ALWAYS read these docs before migrating any component.|If docs missing, run: cd react-frontend && heroui agents-md --migration --output ../AGENTS.md|Entry point: agent-index.mdx|Incremental workflow: (workflows)/agent-guide-incremental.mdx|Full workflow: (workflows)/agent-guide-full.mdx|Cross-cutting: hooks.mdx, styling.mdx|Per-component guides: (components)/<component>.mdx|Migration-for-agents guides: (migration-for-agents)/|Removed v2 components (do NOT migrate unless asked): Code, Image, Navbar, Ripple, Snippet, Spacer, User|useDisclosure → useOverlayState|Collection items now use id+textValue (not key) in v3
