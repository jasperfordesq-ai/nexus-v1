# Contributors

This file lists all contributors to Project NEXUS. This information MUST be displayed on all About pages in accordance with our attribution policy (see [NOTICE](NOTICE)).

## Creator

- **Jasper Ford** - Creator and primary author

## Founders

The originating Irish timebank initiative hOUR Timebank CLG was co-founded by:

- **Jasper Ford**
- **Mary Casey**

## Contributors

- **Steven J. Kelly** - Community insight, product thinking

## Research Foundation

This software is informed by and builds upon a social impact study commissioned by the **West Cork Development Partnership**.

## Acknowledgements

- **West Cork Development Partnership**
- **Fergal Conlon**, SICAP Manager

---

## Machine-Readable Format

For programmatic access, see [contributors.json](contributors.json).

## Attribution Requirements

All public deployments MUST display the contributor list on About pages. See [NOTICE](NOTICE) for full attribution requirements.
