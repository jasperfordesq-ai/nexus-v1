...
<div class="htb-container" style="padding: 40px 20px; background: white; min-height: 500px;">
    ...
    <?php if (isset($_SESSION['user_role']) && $_SESSION['user_role'] === 'admin'): ?>
        <div style="margin-bottom: 20px; text-align: right;">
            <a href="<?= \Nexus\Core\TenantContext::getBasePath() ?>/admin/pages/builder?id=<?= $page['id'] ?>" class="htb-btn htb-btn-secondary" style="font-size: 0.8rem;">✏️ Edit Page</a>
        </div>
    <?php endif; ?>

    <div class="cms-content" style="font-size: 1.1rem; line-height: 1.8; color: #374151;">
        <?= $page['content'] ?>
        <!-- Note: We act as if content is safe HTML (Wysiwyg). In strict environment, use HTMLPurifier. -->
    </div>
</div>
