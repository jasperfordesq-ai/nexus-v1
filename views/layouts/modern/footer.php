...
<style>
    ...
    .nexus-modern-footer {
        ...
        padding: 0;
        overflow: hidden;
        /* Light mode default - glass effect with subtle gradient */
        background: linear-gradient(180deg,
                rgba(249, 250, 251, 0.02) 0%,
                rgba(243, 244, 246, 0.95) 30%,
                rgba(229, 231, 235, 0.98) 100%);
    }

    [data-theme="light"] .nexus-modern-footer {
        background: linear-gradient(180deg,
                rgba(249, 250, 251, 0.02) 0%,
                rgba(243, 244, 246, 0.95) 30%,
                rgba(229, 231, 235, 0.98) 100%);
    ...
    [data-theme="dark"] .nexus-modern-footer {
        background: linear-gradient(180deg,
                rgba(0, 0, 0, 0.02) 0%,
    ...
    .nexus-footer-links a i {
        font-size: 0.8rem;
        opacity: 0.6;
    }

    /* Custom Tenant Message */
    .nexus-footer-message {
    ...
    .nexus-footer-copyright a:hover {
        color: #a5b4fc;
    }
    ...
    /* Hide on mobile - we have bottom nav */
    @media (max-width: 900px) {
        .nexus-modern-footer {
   ...
   ======================================== */
    [data-theme="light"] .nexus-footer-logo-text {
        color: #1f2937;
    }

    [data-theme="light"] .nexus-footer-description {
        color: #64748b;
    }

    [data-theme="light"] .nexus-footer-social a {
        background: rgba(99, 102, 241, 0.08);
        border-color: rgba(99, 102, 241, 0.15);
        color: #6366f1;
    }

    [data-theme="light"] .nexus-footer-social a:hover {
        background: rgba(99, 102, 241, 0.15);
        border-color: rgba(99, 102, 241, 0.3);
        color: #4f46e5;
    }

    [data-theme="light"] .nexus-footer-column h4 {
        color: #1f2937;
    }

    [data-theme="light"] .nexus-footer-links a {
        color: #64748b;
    }

    [data-theme="light"] .nexus-footer-links a:hover {
        color: #6366f1;
    }

    [data-theme="light"] .nexus-footer-message {
        background: linear-gradient(135deg,
    ...
    [data-theme="light"] .nexus-footer-message p {
        color: #374151;
    }
    ...
    [data-theme="light"] .nexus-footer-copyright {
        color: #64748b;
    }

    [data-theme="light"] .nexus-footer-copyright a {
        color: #6366f1;
    }

    [data-theme="light"] .nexus-footer-copyright a:hover {
        color: #4f46e5;
    }

    [data-theme="light"] .nexus-footer-legal a {
        color: #64748b;
    }

    [data-theme="light"] .nexus-footer-legal a:hover {
        color: #6366f1;
    }
</style>
...
<!-- Bottom Navigation (Modern Native Style) -->
<?php
...
$activeTab = 'home';
if (strpos($currentPath, '/messages') !== false) $activeTab = 'messages';
elseif (strpos($currentPath, '/wallet') !== false) $activeTab = 'wallet';
elseif (strpos($currentPath, '/dashboard') !== false) $activeTab = 'dashboard';
elseif (strpos($currentPath, '/profile') !== false) $activeTab = 'profile';
elseif (strpos($currentPath, '/legal') !== false || strpos($currentPath, '/privacy') !== false || strpos($currentPath, '/terms') !== false || strpos($currentPath, '/about') !== false || strpos($currentPath, '/contact') !== false || strpos($currentPath, '/faq') !== false) $activeTab = 'legal';
        ...
        if (class_exists('Nexus\Models\MessageThread')) {
            ...
            foreach ($threads as $thread) {
                if (!empty($thread['unread_count'])) {
                    $unreadMessages += (int)$thread['unread_count'];
                }
            }
        }
    } catch (Exception $e) {
...
?>
<?php
// Get notification count for bottom nav menu button
$menuUnreadNotifs = 0;
if ($isLoggedIn) {
    try {
        if (class_exists('Nexus\Models\Notification')) {
            $menuUnreadNotifs = Nexus\Models\Notification::countUnread($_SESSION['user_id']);
        }
    } catch (Exception $e) {
        $menuUnreadNotifs = 0;
    }
}
$totalBadgeCount = $unreadMessages + $menuUnreadNotifs;
?>
<?php if (false && empty($hideBottomNav)): // DISABLED - Legacy bottom nav replaced by mobile-nav-v2.php 
?>
    <!-- Legacy Navigation (DISABLED - Replaced by mobile-nav-v2.php)
     This entire section is no longer used. The new mobile navigation is in mobile-nav-v2.php
     Kept here for reference only. To permanently remove, delete lines 892-941. -->
    <!--
<nav class="nexus-native-nav" role="navigation" aria-label="Mobile navigation">
    <div class="nexus-nav-pill"></div>
    <div class="nexus-native-nav-inner">
        <a href="<?= $basePath ?>/" class="nexus-native-nav-item <?= $activeTab === 'home' ? 'active' : '' ?>" aria-label="Home" <?= $activeTab === 'home' ? 'aria-current="page"' : '' ?>>
            <i class="fa-solid fa-house" aria-hidden="true"></i>
            <span>Home</span>
        </a>

        <?php if ($isLoggedIn): ?>
            <a href="<?= $basePath ?>/messages" class="nexus-native-nav-item <?= $activeTab === 'messages' ? 'active' : '' ?>" aria-label="Messages<?= $unreadMessages > 0 ? ', ' . $unreadMessages . ' unread' : '' ?>" <?= $activeTab === 'messages' ? 'aria-current="page"' : '' ?>>
                <i class="fa-solid fa-message" aria-hidden="true"></i>
                <span>Messages</span>
                <?php if ($unreadMessages > 0): ?>
                    <span class="nexus-native-nav-badge" aria-hidden="true"><?= $unreadMessages > 9 ? '9+' : $unreadMessages ?></span>
                <?php endif; ?>
            </a>

            <a href="<?= $basePath ?>/wallet" class="nexus-native-nav-item <?= $activeTab === 'wallet' ? 'active' : '' ?>" aria-label="Wallet" <?= $activeTab === 'wallet' ? 'aria-current="page"' : '' ?>>
                <i class="fa-solid fa-wallet" aria-hidden="true"></i>
                <span>Wallet</span>
            </a>

            <button type="button" class="nexus-native-nav-item" aria-label="Menu<?= $totalBadgeCount > 0 ? ', ' . $totalBadgeCount . ' notifications' : '' ?>" onclick="toggleNexusDrawer()">
                <i class="fa-solid fa-bars" aria-hidden="true"></i>
                <span>Menu</span>
                <?php if ($totalBadgeCount > 0): ?>
                    <span class="nexus-native-nav-badge" aria-hidden="true"><?= $totalBadgeCount > 9 ? '9+' : $totalBadgeCount ?></span>
                <?php endif; ?>
            </button>
        <?php else: ?>
            <a href="<?= $basePath ?>/listings" class="nexus-native-nav-item" aria-label="Browse listings">
                <i class="fa-solid fa-gift" aria-hidden="true"></i>
                <span>Listings</span>
            </a>

            <a href="<?= $basePath ?>/groups" class="nexus-native-nav-item" aria-label="Browse community hubs">
                <i class="fa-solid fa-users" aria-hidden="true"></i>
                <span>Hubs</span>
            </a>

            <button type="button" class="nexus-native-nav-item" aria-label="Menu" onclick="toggleNexusDrawer()">
                <i class="fa-solid fa-bars" aria-hidden="true"></i>
                <span>Menu</span>
            </button>
        <?php endif; ?>
