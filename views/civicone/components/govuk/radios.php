 ...
 * Reusable radio buttons following GOV.UK Design System v5.14.0
 *
 * @param string $name - Input name attribute
 * @param array $items - Array of radio items with 'value', 'label', 'hint' (optional), 'checked' (optional)
 ...
 * @param bool $inline - Display radios inline
 * @param bool $small - Use small variant
 * @param string $class - Additional CSS classes
 *
 * Usage:
 * <?php include __DIR__ . '/radios.php'; echo civicone_govuk_radios([
 *     'name' => 'contact',
...
function civicone_govuk_radios($args = []) {
    $defaults = [
        'name' => '',
        'items' => [],
        'legend' => '',
    ...
    // Radios container classes
    $radiosClasses = ['govuk-radios'];
    if ($args['inline']) {
        $radiosClasses[] = 'govuk-radios--inline';
    }
    if ($args['small']) {
        $radiosClasses[] = 'govuk-radios--small';
    }
    if (!empty($args['class'])) {
        $radiosClasses[] = $args['class'];
    }
    ...
    // Radios
    $html .= '<div class="' . implode(' ', $radiosClasses) . '" data-module="govuk-radios">';
    ...
    foreach ($args['items'] as $index => $item) {
        ...
        if (isset($item['divider'])) {
            $html .= '<div class="govuk-radios__divider">' . htmlspecialchars($item['divider']) . '</div>';
            continue;
        }
        ...
        // Input
        $html .= '<input class="govuk-radios__input" ';
        ...
        // Label
        $html .= '<label class="govuk-label govuk-radios__label" for="' . htmlspecialchars($itemId) . '">';
        $html .= htmlspecialchars($item['label'] ?? '');
        $html .= '</label>';

        // Item hint
        if (!empty($item['hint'])) {
            $html .= '<div id="' . htmlspecialchars($itemHintId) . '" class="govuk-hint govuk-radios__hint">';
