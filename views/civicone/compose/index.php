...
<head>
    ...
    <!-- Compose Page Styles -->
    <link rel="stylesheet" href="<?= $basePath ?>/assets/css/civicone-compose-index.min.css">
    ...
    function nextListingStep(step) {
        ...
        if (step === 2) {
            if (!title || !title.value.trim()) {
                if (title) {
                    title.focus();
                    title.classList.add('md-input--error', 'shake');
                    setTimeout(function() { title.classList.remove('md-input--error', 'shake'); }, 400);
                }
                return;
            }
            if (!desc || !desc.value.trim()) {
                if (desc) {
                    desc.focus();
                    desc.classList.add('md-input--error', 'shake');
                    setTimeout(function() { desc.classList.remove('md-input--error', 'shake'); }, 400);
                }
                return;
            }
        }
    ...
    function previewImage(input, type) {
        if (input.files && input.files[0]) {
            ...
            reader.onload = function(e) {
                ...
                document.querySelector('#' + type + '-image-upload .md-image-placeholder').classList.add('hidden');
    ...
    function removeImage(type) {
        ...
        document.querySelector('#' + type + '-image-upload .md-image-placeholder').classList.remove('hidden');
    ...
    // SDG Toggle - uses CSS custom property for dynamic color
    function toggleSDG(checkbox, color) {
        var card = checkbox.closest('.holo-sdg-card');
        if (checkbox.checked) {
            card.style.setProperty('--sdg-color', color);
            card.classList.add('sdg-selected');
        } else {
            card.classList.remove('sdg-selected');
        }
        haptic();
    }
    </script>
...
<body class="compose-page">
    <!-- Styles loaded from /assets/css/civicone-compose-index.min.css -->
    <!-- NOTE: 1900 lines of inline CSS removed - now in external file -->
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (groupSearch) {
            ...
            groupItems.forEach(item => {
                item.addEventListener('click', function() {
                    ...
                    selectedGroupDisplay.classList.add('visible');
                    groupSearch.value = '';
                    groupResults.classList.remove('visible');
        ...
        // ============================================
        // Scroll lock handled by .compose-page class in external CSS
