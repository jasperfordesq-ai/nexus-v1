 ...
 * 
 * Purpose: Display organization profile with glassmorphism design
...
?>
<link rel="stylesheet" href="<?= $base ?>/assets/css/purged/civicone-volunteering-show-org.min.css">
        ...
        <div class="org-profile-content">
            ...
            <div class="org-profile-info">
                ...
                <span class="org-profile-status <?= ($org['status'] ?? 'active') === 'active' ? 'active' : 'pending' ?>">
                    <i class="fa-solid <?= ($org['status'] ?? 'active') === 'active' ? 'fa-circle-check' : 'fa-clock' ?>" aria-hidden="true"></i>
                ...
                <div class="org-quick-actions">
                    <?php if ($hasTimebanking): ?>
                        <?php if ($isMember): ?>
                            <a href="<?= $base ?>/organizations/<?= $org['id'] ?>/wallet" class="org-action-btn org-action-btn--wallet">
                                <i class="fa-solid fa-wallet" aria-hidden="true"></i> View Wallet
                            </a>
                        <?php elseif ($isLoggedIn): ?>
                            <form action="<?= $base ?>/organizations/<?= $org['id'] ?>/members/request" method="POST" class="d-inline">
                                <?= \Nexus\Core\Csrf::input() ?>
                                <button type="submit" class="org-action-btn org-action-btn--primary">
                                    <i class="fa-solid fa-user-plus" aria-hidden="true"></i> Request to Join
                                </button>
                            </form>
                        <?php endif; ?>
                    <?php endif; ?>
                    ...
                    <?php if (!empty($org['website'])): ?>
                        <a href="<?= htmlspecialchars($org['website']) ?>" target="_blank" class="org-action-btn org-action-btn--secondary">
                            <i class="fa-solid fa-external-link" aria-hidden="true"></i> Visit Website
                        </a>
                    <?php endif; ?>

                    <?php if ($isAdmin): ?>
                        <a href="<?= $base ?>/volunteering/opportunities/create?org=<?= $org['id'] ?>" class="org-action-btn org-action-btn--secondary">
                            <i class="fa-solid fa-plus" aria-hidden="true"></i> Add Opportunity
                        </a>
                    <?php endif; ?>
                </div>
            </div>
    ...
    <div class="org-section">
        ...
        <?php else: ?>
            <div class="org-empty-state">
                ...
                <?php if ($isAdmin): ?>
                    <a href="<?= $base ?>/volunteering/opportunities/create?org=<?= $org['id'] ?>" class="org-action-btn org-action-btn--primary mt-20">
                        <i class="fa-solid fa-plus" aria-hidden="true"></i> Create First Opportunity
                    </a>
                <?php endif; ?>
            </div>
        <?php endif; ?>
