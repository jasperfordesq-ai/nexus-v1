...
- **Listings proximity filter ("Near me") was completely ignored.** The `getAll()` method accepted `near_lat`/`near_lng`/`radius_km` filters from the controller but never applied them — all listings were returned regardless of distance. Users in Dublin saw listings from Cork when filtering to 2 km. The fix delegates to the existing `getNearby()` haversine query whenever proximity params are present, and excludes proximity-filtered requests from the Meilisearch shortcut path so the SQL distance calculation always runs.
