...
- **SendGrid event webhook (`POST /api/v2/webhooks/sendgrid/events`).** Receives `delivered` / `bounce` / `dropped` / `open` / `click` / `spamreport` / `unsubscribe` events from SendGrid in real time, updates `email_log` (delivered_at / bounced_at / opened_at + status), and upserts `email_suppression`. Authenticated via SendGrid's ECDSA signature (preferred — `SENDGRID_EVENT_WEBHOOK_PUBLIC_KEY`) with a shared-secret token fallback (`SENDGRID_WEBHOOK_TOKEN`). The Mailer captures the `X-Message-Id` header from SendGrid on send and writes it to `email_log.provider_message_id` so webhook events match back to log rows. Net result: real-time delivery / bounce / open tracking per recipient, instead of "we know we submitted, we don't know what happened next."
