...
- **Courses module (Alpha) — community learning.** A new self-contained, tenant-scoped learning module: courses organised into sections and lessons (video, rich text, PDF, and external embeds), free and members-only enrollment, per-lesson progress tracking with automatic course completion, and auto-graded multiple-choice quizzes. Members granted an **instructor** role (alongside admins) can author courses through a course builder; admins get a moderation queue, instructor-grant management, categories, and tenant analytics. The module ships **marked "Alpha"** in module configuration and is **off by default** — each community opts in per tenant. An "Alpha" badge is shown on the member-facing Courses pages. Course completion awards gamification XP and a graduate badge; deeper integrations (learn-to-earn / teach-to-earn time credits, feed celebration posts, group linkage, certificates, drip content, cohorts) are scaffolded for later phases. Fully translated across all 11 languages.
