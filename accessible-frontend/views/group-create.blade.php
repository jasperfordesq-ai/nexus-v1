...
@section('content')
    ...
    @if (($status ?? null) === 'group-create-failed')
        <div class="govuk-error-summary" data-module="govuk-error-summary" tabindex="-1">
            <div role="alert">
                <h2 class="govuk-error-summary__title">{{ __('govuk_alpha.polish_groups.create_failed_heading') }}</h2>
                <div class="govuk-error-summary__body">
                    <p class="govuk-body">{{ __('govuk_alpha.groups.create.failed') }}</p>
                </div>
            </div>
        </div>
    @endif
    ...
    <form method="post" action="{{ route('govuk-alpha.groups.store', ['tenantSlug' => $tenantSlug]) }}" enctype="multipart/form-data" novalidate>
        ...
        <div class="govuk-form-group">
            ...
            <input class="govuk-input govuk-!-width-two-thirds" id="location" name="location" type="text" maxlength="255" value="{{ old('location') }}" aria-describedby="location-hint">
        </div>
