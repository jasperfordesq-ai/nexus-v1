...
@section('content')
    ...
    <form method="post" action="{{ route('govuk-alpha.listings.store', ['tenantSlug' => $tenantSlug]) }}" enctype="multipart/form-data" novalidate>
        ...
        <div class="govuk-form-group{{ $errors->has('description') ? ' govuk-form-group--error' : '' }}">
            ...
            @enderror
            <textarea class="govuk-textarea{{ $errors->has('description') ? ' govuk-textarea--error' : '' }}" id="description" name="description" rows="6" aria-describedby="{{ $describedBy('description', 'description-hint') }}">{{ old('description') }}</textarea>
        </div>
