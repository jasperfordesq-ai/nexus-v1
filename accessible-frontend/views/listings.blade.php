...
@section('content')
    ...
    @if ($moduleDisabled)
        <div class="govuk-notification-banner" data-module="govuk-notification-banner" role="region" aria-labelledby="listings-disabled-title">
            <div class="govuk-notification-banner__header">
                    ...
                    <div class="govuk-grid-column-one-quarter">
                        <div class="govuk-form-group">
                            ...
                            @if (!empty($filters['near_no_location']))
                                <p id="near-hint" class="govuk-hint govuk-!-margin-top-1">{{ __('govuk_alpha.near_me.no_location') }}</p>
                            @endif
                        </div>
                    </div>
                </div>
            ...
            @if (!empty($meta['has_more']) && !empty($meta['cursor']))
                <nav class="govuk-pagination govuk-pagination--block govuk-!-margin-top-6" aria-label="{{ __('govuk_alpha.listings.pagination_label') }}">
                    <div class="govuk-pagination__next">
                        <a class="govuk-link govuk-pagination__link" href="{{ route('govuk-alpha.listings.index', array_filter(['tenantSlug' => $tenantSlug, 'q' => $filters['search'] ?? null, 'type' => $filters['type'] ?? null, 'category_id' => $filters['category_id'] ?? null, 'hours' => ($filters['hours'] ?? 'any') !== 'any' ? $filters['hours'] : null, 'service' => ($filters['service'] ?? 'any') !== 'any' ? $filters['service'] : null, 'near' => ($filters['near'] ?? 'any') !== 'any' ? $filters['near'] : null, 'posted' => ($filters['posted'] ?? 'any') !== 'any' ? $filters['posted'] : null, 'sort' => ($filters['sort'] ?? 'newest') !== 'newest' ? $filters['sort'] : null, 'cursor' => $meta['cursor']])) }}" rel="next">
