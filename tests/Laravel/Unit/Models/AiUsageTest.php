<?php
// Copyright © 2024–2026 Jasper Ford
// SPDX-License-Identifier: AGPL-3.0-or-later
// Author: Jasper Ford
// See NOTICE file for attribution and acknowledgements.

namespace Tests\Laravel\Unit\Models;

use App\Models\AiUsage;
use App\Models\Concerns\HasTenantScope;
use Illuminate\Database\Eloquent\Relations\BelongsTo;
use Tests\Laravel\TestCase;

class AiUsageTest extends TestCase
{
    public function test_table_name(): void
    {
        $model = new AiUsage();
        $this->assertEquals('ai_usage', $model->getTable());
    }

    public function test_fillable_contains_expected_fields(): void
    {
        $model = new AiUsage();
        $expected = [
            'tenant_id', 'user_id', 'provider', 'feature',
            'tokens_input', 'tokens_output', 'cost_usd',
        ];
        $this->assertEquals($expected, $model->getFillable());
    }

    public function test_casts_contain_correct_types(): void
    {
        $model = new AiUsage();
        $casts = $model->getCasts();
        $this->assertEquals('integer', $casts['tokens_input']);
        $this->assertEquals('integer', $casts['tokens_output']);
        $this->assertEquals('decimal:6', $casts['cost_usd']);
    }

    public function test_uses_has_tenant_scope_trait(): void
    {
        $this->assertContains(
            HasTenantScope::class,
            class_uses_recursive(AiUsage::class)
        );
    }

    public function test_user_relationship_returns_belongs_to(): void
    {
        $model = new AiUsage();
        $this->assertInstanceOf(BelongsTo::class, $model->user());
    }
}
