 ...
 */
class TransactionLimitServiceTest extends \Tests\Laravel\TestCase
{
    ...
    protected static function createTestData(): void
    {
        ...
        Database::query(
            "INSERT INTO users (tenant_id, email, username, first_name, last_name, name, balance, is_approved, created_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, 1, NOW())",
            [self::$staticTenantId, "limit_test_{$timestamp}@test.com", "limit_test_{$timestamp}", 'Limit', 'Test', 'Limit Test', 1000]
        );
        ...
        try {
            Database::query(
                "INSERT INTO vol_organizations (tenant_id, user_id, name, slug, status, created_at)
                 VALUES (?, ?, ?, ?, 'active', NOW())",
                [self::$staticTenantId, self::$testUserId, "Test Org {$timestamp}", "test-org-{$timestamp}"]
            );
