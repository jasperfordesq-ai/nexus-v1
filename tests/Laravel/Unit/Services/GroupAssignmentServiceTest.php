 ...
 */
class GroupAssignmentServiceTest extends \Tests\Laravel\TestCase
{
    ...
    protected static function createTestData(): void
    {
        ...
        Database::query(
            "INSERT INTO users (tenant_id, email, username, first_name, last_name, name, location, created_at)
             VALUES (?, ?, ?, ?, ?, ?, ?, NOW())",
            [self::$staticTenantId, "grpassign_{$ts}@test.com", "grpassign_{$ts}", 'Assign', 'User', 'Assign User', 'Dublin, Ireland']
        );
        ...
        Database::query(
            "INSERT INTO `groups` (tenant_id, name, description, owner_id, created_at)
             VALUES (?, ?, ?, ?, NOW())",
            [self::$staticTenantId, "Dublin {$ts}", 'Dublin hub group', self::$testUserId]
        );
