 ...
 */
class OptimizedGroupQueriesTest extends \Tests\Laravel\TestCase
{
    ...
    protected static function createTestData(): void
    {
        ...
        Database::query(
            "INSERT INTO users (tenant_id, email, username, first_name, last_name, name, created_at)
             VALUES (?, ?, ?, ?, ?, ?, NOW())",
            [self::$staticTenantId, "optgrp_{$ts}@test.com", "optgrp_{$ts}", 'Opt', 'User', 'Opt User']
        );
        ...
        Database::query(
            "INSERT INTO `groups` (tenant_id, name, description, owner_id, created_at)
             VALUES (?, ?, ?, ?, NOW())",
            [self::$staticTenantId, "Parent Group {$ts}", 'Test parent group', self::$testUserId]
        );
        ...
        Database::query(
            "INSERT INTO `groups` (tenant_id, name, description, owner_id, parent_id, created_at)
             VALUES (?, ?, ?, ?, ?, NOW())",
            [self::$staticTenantId, "Child Group {$ts}", 'Test child group', self::$testUserId, self::$testParentGroupId]
        );
