...
/**
 * Integration tests for VolunteeringController
 *
 * Tests volunteering module configuration.
 *
 * @group integration
 * @group admin
 */
class VolunteeringControllerTest extends ApiTestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        $adminEmail = 'Volunteering_admin_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'Volunteering Admin', 'Volunteering', 'Admin', 'admin', 'active', 1, NOW())",
        ...
        $memberEmail = 'Volunteering_member_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'Volunteering Member', 'Volunteering', 'Member', 'member', 'active', 1, NOW())",
    ...
    public function testSaveUpdatesSettings(): void
    {
        $response = $this->makeApiRequest(
            'POST',
            '/admin-legacy/Volunteering/save',
            ['setting' => 'value'],
            ['Authorization' => 'Bearer ' . self::$adminToken]
        );
    ...
    {
        $endpoints = [
            ['GET', '/admin-legacy/Volunteering'],
            ['POST', '/admin-legacy/Volunteering/save'],
        ];
