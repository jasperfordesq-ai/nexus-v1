...
/**
 * Integration tests for MatchApprovalsController
 *
 * Tests match approval workflow for brokers.
 *
 * @group integration
 * @group admin
 */
class MatchApprovalsControllerTest extends ApiTestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        $adminEmail = 'MatchApprovals_admin_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'MatchApprovals Admin', 'MatchApprovals', 'Admin', 'admin', 'active', 1, NOW())",
        ...
        $memberEmail = 'MatchApprovals_member_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'MatchApprovals Member', 'MatchApprovals', 'Member', 'member', 'active', 1, NOW())",
    ...
    public function testSaveUpdatesSettings(): void
    {
        $response = $this->makeApiRequest(
            'POST',
            '/admin-legacy/MatchApprovals/save',
            ['setting' => 'value'],
            ['Authorization' => 'Bearer ' . self::$adminToken]
        );
    ...
    {
        $endpoints = [
            ['GET', '/admin-legacy/MatchApprovals'],
            ['POST', '/admin-legacy/MatchApprovals/save'],
        ];
