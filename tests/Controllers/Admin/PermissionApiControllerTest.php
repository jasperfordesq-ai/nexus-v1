...
/**
 * Integration tests for PermissionApiController
 *
 * Tests permission management API.
 *
 * @group integration
 * @group admin
 */
class PermissionApiControllerTest extends ApiTestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        $adminEmail = 'PermissionApi_admin_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'PermissionApi Admin', 'PermissionApi', 'Admin', 'admin', 'active', 1, NOW())",
        ...
        $memberEmail = 'PermissionApi_member_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'PermissionApi Member', 'PermissionApi', 'Member', 'member', 'active', 1, NOW())",
    ...
    public function testSaveUpdatesSettings(): void
    {
        $response = $this->makeApiRequest(
            'POST',
            '/admin-legacy/PermissionApi/save',
            ['setting' => 'value'],
            ['Authorization' => 'Bearer ' . self::$adminToken]
        );
    ...
    {
        $endpoints = [
            ['GET', '/admin-legacy/PermissionApi'],
            ['POST', '/admin-legacy/PermissionApi/save'],
        ];
