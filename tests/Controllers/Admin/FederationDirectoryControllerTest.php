...
/**
 * Integration tests for FederationDirectoryController
 *
 * Tests federation directory listings.
 *
 * @group integration
 * @group admin
 */
class FederationDirectoryControllerTest extends ApiTestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        $adminEmail = 'FederationDirectory_admin_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'FederationDirectory Admin', 'FederationDirectory', 'Admin', 'admin', 'active', 1, NOW())",
        ...
        $memberEmail = 'FederationDirectory_member_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'FederationDirectory Member', 'FederationDirectory', 'Member', 'member', 'active', 1, NOW())",
    ...
    public function testIndexShowsInterface(): void
    {
        $response = $this->makeApiRequest(
            'GET',
            '/admin-legacy/FederationDirectory',
            [],
    ...
    public function testSaveUpdatesSettings(): void
    {
        $response = $this->makeApiRequest(
            'POST',
            '/admin-legacy/FederationDirectory/save',
            ['setting' => 'value'],
            ['Authorization' => 'Bearer ' . self::$adminToken]
        );
    ...
    {
        $endpoints = [
            ['GET', '/admin-legacy/FederationDirectory'],
            ['POST', '/admin-legacy/FederationDirectory/save'],
        ];
