...
/**
 * Integration tests for LegalDocumentsController
 *
 * Tests legal document management and versioning.
 *
 * @group integration
 * @group admin
 */
class LegalDocumentsControllerTest extends ApiTestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        $adminEmail = 'LegalDocuments_admin_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'LegalDocuments Admin', 'LegalDocuments', 'Admin', 'admin', 'active', 1, NOW())",
        ...
        $memberEmail = 'LegalDocuments_member_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'LegalDocuments Member', 'LegalDocuments', 'Member', 'member', 'active', 1, NOW())",
    ...
    public function testSaveUpdatesSettings(): void
    {
        $response = $this->makeApiRequest(
            'POST',
            '/admin-legacy/LegalDocuments/save',
            ['setting' => 'value'],
            ['Authorization' => 'Bearer ' . self::$adminToken]
        );
    ...
    {
        $endpoints = [
            ['GET', '/admin-legacy/LegalDocuments'],
            ['POST', '/admin-legacy/LegalDocuments/save'],
        ];
