...
/**
 * Integration tests for FederationImportController
 *
 * Tests federation data import functionality.
 *
 * @group integration
 * @group admin
 */
class FederationImportControllerTest extends ApiTestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        $adminEmail = 'FederationImport_admin_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'FederationImport Admin', 'FederationImport', 'Admin', 'admin', 'active', 1, NOW())",
        ...
        $memberEmail = 'FederationImport_member_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'FederationImport Member', 'FederationImport', 'Member', 'member', 'active', 1, NOW())",
    ...
    public function testSaveUpdatesSettings(): void
    {
        $response = $this->makeApiRequest(
            'POST',
            '/admin-legacy/FederationImport/save',
            ['setting' => 'value'],
            ['Authorization' => 'Bearer ' . self::$adminToken]
        );
    ...
    {
        $endpoints = [
            ['GET', '/admin-legacy/FederationImport'],
            ['POST', '/admin-legacy/FederationImport/save'],
        ];
