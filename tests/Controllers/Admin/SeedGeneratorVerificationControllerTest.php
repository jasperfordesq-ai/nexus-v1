...
/**
 * Integration tests for SeedGeneratorVerificationController
 *
 * Tests seed data verification.
 *
 * @group integration
 * @group admin
 */
class SeedGeneratorVerificationControllerTest extends ApiTestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        $adminEmail = 'SeedGeneratorVerification_admin_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'SeedGeneratorVerification Admin', 'SeedGeneratorVerification', 'Admin', 'admin', 'active', 1, NOW())",
        ...
        $memberEmail = 'SeedGeneratorVerification_member_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'SeedGeneratorVerification Member', 'SeedGeneratorVerification', 'Member', 'member', 'active', 1, NOW())",
    ...
    public function testIndexShowsInterface(): void
    {
        $response = $this->makeApiRequest(
            'GET',
            '/admin-legacy/SeedGeneratorVerification',
            [],
    ...
    public function testSaveUpdatesSettings(): void
    {
        $response = $this->makeApiRequest(
            'POST',
            '/admin-legacy/SeedGeneratorVerification/save',
            ['setting' => 'value'],
            ['Authorization' => 'Bearer ' . self::$adminToken]
        );
    ...
    {
        $endpoints = [
            ['GET', '/admin-legacy/SeedGeneratorVerification'],
            ['POST', '/admin-legacy/SeedGeneratorVerification/save'],
        ];
