...
/**
 * Integration tests for FederationSettingsController
 *
 * Tests federation configuration settings.
 *
 * @group integration
 * @group admin
 */
class FederationSettingsControllerTest extends ApiTestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        $adminEmail = 'FederationSettings_admin_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'FederationSettings Admin', 'FederationSettings', 'Admin', 'admin', 'active', 1, NOW())",
        ...
        $memberEmail = 'FederationSettings_member_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'FederationSettings Member', 'FederationSettings', 'Member', 'member', 'active', 1, NOW())",
    ...
    public function testIndexShowsInterface(): void
    {
        $response = $this->makeApiRequest(
            'GET',
            '/admin-legacy/FederationSettings',
            [],
    ...
    public function testSaveUpdatesSettings(): void
    {
        $response = $this->makeApiRequest(
            'POST',
            '/admin-legacy/FederationSettings/save',
            ['setting' => 'value'],
            ['Authorization' => 'Bearer ' . self::$adminToken]
        );
    ...
    {
        $endpoints = [
            ['GET', '/admin-legacy/FederationSettings'],
            ['POST', '/admin-legacy/FederationSettings/save'],
        ];
