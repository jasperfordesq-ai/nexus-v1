...
/**
 * Integration tests for TimebankingController
 *
 * Tests timebanking configuration and settings.
 *
 * @group integration
 * @group admin
 */
class TimebankingControllerTest extends ApiTestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        $adminEmail = 'Timebanking_admin_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'Timebanking Admin', 'Timebanking', 'Admin', 'admin', 'active', 1, NOW())",
        ...
        $memberEmail = 'Timebanking_member_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'Timebanking Member', 'Timebanking', 'Member', 'member', 'active', 1, NOW())",
    ...
    {
        $endpoints = [
            ['GET', '/admin-legacy/Timebanking'],
            ['POST', '/admin-legacy/Timebanking/save'],
        ];
