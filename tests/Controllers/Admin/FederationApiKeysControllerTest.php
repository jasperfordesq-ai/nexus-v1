...
/**
 * Integration tests for FederationApiKeysController
 *
 * Tests federation API key management.
 *
 * @group integration
 * @group admin
 */
class FederationApiKeysControllerTest extends ApiTestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        $adminEmail = 'FederationApiKeys_admin_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'FederationApiKeys Admin', 'FederationApiKeys', 'Admin', 'admin', 'active', 1, NOW())",
        ...
        $memberEmail = 'FederationApiKeys_member_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'FederationApiKeys Member', 'FederationApiKeys', 'Member', 'member', 'active', 1, NOW())",
    ...
    public function testIndexShowsInterface(): void
    {
        $response = $this->makeApiRequest(
            'GET',
            '/admin-legacy/FederationApiKeys',
            [],
    ...
    public function testSaveUpdatesSettings(): void
    {
        $response = $this->makeApiRequest(
            'POST',
            '/admin-legacy/FederationApiKeys/save',
            ['setting' => 'value'],
            ['Authorization' => 'Bearer ' . self::$adminToken]
        );
    ...
    {
        $endpoints = [
            ['GET', '/admin-legacy/FederationApiKeys'],
            ['POST', '/admin-legacy/FederationApiKeys/save'],
        ];
