...
/**
 * Integration tests for CustomBadgeController
 *
 * Tests custom badge creation and management.
 *
 * @group integration
 * @group admin
 */
class CustomBadgeControllerTest extends ApiTestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        $adminEmail = 'CustomBadge_admin_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'CustomBadge Admin', 'CustomBadge', 'Admin', 'admin', 'active', 1, NOW())",
        ...
        $memberEmail = 'CustomBadge_member_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'CustomBadge Member', 'CustomBadge', 'Member', 'member', 'active', 1, NOW())",
    ...
    {
        $endpoints = [
            ['GET', '/admin-legacy/CustomBadge'],
            ['POST', '/admin-legacy/CustomBadge/save'],
        ];
