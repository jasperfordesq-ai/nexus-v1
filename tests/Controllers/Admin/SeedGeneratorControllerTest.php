...
/**
 * Integration tests for SeedGeneratorController
 *
 * Tests database seeding and test data generation.
 *
 * @group integration
 * @group admin
 */
class SeedGeneratorControllerTest extends ApiTestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        $adminEmail = 'SeedGenerator_admin_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'SeedGenerator Admin', 'SeedGenerator', 'Admin', 'admin', 'active', 1, NOW())",
        ...
        $memberEmail = 'SeedGenerator_member_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'SeedGenerator Member', 'SeedGenerator', 'Member', 'member', 'active', 1, NOW())",
    ...
    public function testSaveUpdatesSettings(): void
    {
        $response = $this->makeApiRequest(
            'POST',
            '/admin-legacy/SeedGenerator/save',
            ['setting' => 'value'],
            ['Authorization' => 'Bearer ' . self::$adminToken]
        );
    ...
    {
        $endpoints = [
            ['GET', '/admin-legacy/SeedGenerator'],
            ['POST', '/admin-legacy/SeedGenerator/save'],
        ];
