...
/**
 * Integration tests for FederationExportController
 *
 * Tests federation data export functionality.
 *
 * @group integration
 * @group admin
 */
class FederationExportControllerTest extends ApiTestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        $adminEmail = 'FederationExport_admin_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'FederationExport Admin', 'FederationExport', 'Admin', 'admin', 'active', 1, NOW())",
        ...
        $memberEmail = 'FederationExport_member_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'FederationExport Member', 'FederationExport', 'Member', 'member', 'active', 1, NOW())",
    ...
    public function testSaveUpdatesSettings(): void
    {
        $response = $this->makeApiRequest(
            'POST',
            '/admin-legacy/FederationExport/save',
            ['setting' => 'value'],
            ['Authorization' => 'Bearer ' . self::$adminToken]
        );
    ...
    {
        $endpoints = [
            ['GET', '/admin-legacy/FederationExport'],
            ['POST', '/admin-legacy/FederationExport/save'],
        ];
