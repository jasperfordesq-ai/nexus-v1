...
/**
 * Integration tests for SmartMatchingController
 *
 * Tests AI-powered matching configuration.
 *
 * @group integration
 * @group admin
 */
class SmartMatchingControllerTest extends ApiTestCase
{
    ...
    public static function setUpBeforeClass(): void
    {
        ...
        $adminEmail = 'SmartMatching_admin_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'SmartMatching Admin', 'SmartMatching', 'Admin', 'admin', 'active', 1, NOW())",
        ...
        $memberEmail = 'SmartMatching_member_' . uniqid() . '@test.local';
        Database::query(
            "INSERT INTO users (tenant_id, email, password_hash, name, first_name, last_name, role, status, is_approved, created_at)
             VALUES (?, ?, ?, 'SmartMatching Member', 'SmartMatching', 'Member', 'member', 'active', 1, NOW())",
    ...
    public function testSaveUpdatesSettings(): void
    {
        $response = $this->makeApiRequest(
            'POST',
            '/admin-legacy/SmartMatching/save',
            ['setting' => 'value'],
            ['Authorization' => 'Bearer ' . self::$adminToken]
        );
    ...
    {
        $endpoints = [
            ['GET', '/admin-legacy/SmartMatching'],
            ['POST', '/admin-legacy/SmartMatching/save'],
        ];
