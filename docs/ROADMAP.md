...
| AG77 | Sub-regional geographic units | Roadmap | Finer-grained geographic subdivision within a canton tenant (Quartier/Ortsteil layers) — beyond `ProximityFilter`'s radius approach. Deferred: no specification yet, needs canton-by-canton address-data agreement. Flagged as honest gap for evaluation conversation. |
