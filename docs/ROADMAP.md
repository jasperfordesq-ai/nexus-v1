...
| AG80 | FADP/nDSG disclosure and retention pack | Critical | Convert the current trust/compliance notes into a concrete pilot disclosure pack: controller/processor matrix, data categories, retention defaults, export/deletion handling, research consent, federation aggregate policy, isolated-node option, subprocessors, and incident-response owner. This is documentation and configuration first; formal legal certification remains external. |
| AG81 | KISS operating-policy workshop | High | Decide the human rules that software cannot guess: who may approve hours, trusted-reviewer threshold, SLA windows, legacy-hour settlement policy, reciprocal-balance intervention policy, safeguarding escalation owners, CHF social-value methodology, and statement cadence. Output should be tenant settings plus a signed policy appendix for the pilot. |
...
| AG83 | Pilot success metrics and baseline capture | High | Define the first pilot scoreboard before onboarding residents: active members, first-response time, approved hours, recurring support relationships, coordinator workload, satisfaction, social isolation proxy, municipal communication reach, business participation, and cost-offset assumptions. Use AG66/AG76 baseline and ROI surfaces, but require a pre-pilot baseline label and quarterly review cadence. |
