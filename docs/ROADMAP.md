...
| AG4 | Build formal Caring Community module profile | Critical | Strong partial: feature flag, preset, admin switch, public hub route, nav gate, quick-create gate, dashboard entry points, admin configuration detail page, municipal reporting, KISS workflow console, installable KISS role pack, tenant-level workflow policy controls, SLA-driven review queue, policy-aware municipal reporting, approval/self-log policy enforcement in hour submission, auto-payment parity for auto-approved logs, saved municipal report templates, member statement API/UI/export affordances, recurring support relationship API/UI, relationship-linked hour logging, optional organisation support for relationship logs, KISS review approve/decline actions, coordinator assignment/escalation actions, and searchable member lookup for relationship creation are implemented. Next: member-facing recurring support visibility, support relationship check-in workflow polish, and browser-level switch-off verification. |
| AG5 | Add kill-switch regression tests | Critical | Strong partial: quick-create, route config, community nav, dashboard feature-gating, municipal impact export affordance tests, and Caring Community admin endpoint smoke coverage are implemented. Next: admin toggle end-to-end coverage and browser-level verification that visible UI disappears immediately after switch-off. |
