...
| AG77 | ⚠️ Sub-regional geographic units | Backend foundation 2026-04-30 | Added tenant-scoped `caring_sub_regions` for Quartier/Ortsteil/municipality/canton subdivisions, with active/inactive status, postal-code metadata, optional centroid/boundary payloads, member-readable active listing, admin CRUD APIs, and care-provider `sub_region_id` filtering. This gives Swiss pilots a practical first layer below canton/municipality without requiring authoritative boundary imports. Remaining work: translated admin UI, member filter controls beyond the provider directory API, and canton-by-canton address-data agreements before polygon-based routing. |
