...
| AG5 | Add kill-switch regression tests | Critical | Strong partial: quick-create, route config, community nav, dashboard feature-gating, and municipal impact export affordance tests are implemented. Next: admin toggle end-to-end coverage and route-level smoke coverage for every Caring Community admin endpoint. |
