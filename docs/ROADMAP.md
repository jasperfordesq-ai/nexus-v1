...
| AG28 | A1 — Regional points (third currency) | High | 🟡 Foundation built (2026-04-28) — isolated regional-points wallet added behind `caring_community.regional_points.enabled`, default OFF. New standalone tables `caring_regional_point_accounts` + `caring_regional_point_transactions`; no changes to `users.points`, timebank `users.balance`, hour approval, or marketplace checkout. Member APIs: summary/history. Admin APIs: config, ledger, issue, adjust. Tests cover off-by-default, no timebank balance mutation, ledger writes, and tenant isolation. Still pending: optional auto-issuance hook per approved hour, member-to-member transfers, and marketplace redemption hook. |
