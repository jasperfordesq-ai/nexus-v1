...
| TD12 | Delete `AdminPlaceholder` stub component | ✅ Done | Low | Confirmed 2026-04-30: no `AdminPlaceholder` component remains and no tests import it. Only replacement notes remain in implemented matching modules. |
| TD13 | Revisit dev/prod PHP memory asymmetry | ✅ Done | Low | Confirmed 2026-04-30: `Dockerfile` and `Dockerfile.prod` both set PHP `memory_limit=1G`, with comments directing heavy artisan tasks to `php -d memory_limit=-1`. Dev/prod PHP memory config is now aligned. |
| TD14 | Monitor prod container memory after 512M→768M bump | ✅ Done | Low | Superseded 2026-04-30: `compose.yml` and `compose.prod.yml` now give the PHP app a 2G container limit with 512M reservation, aligned with PHP `memory_limit=1G` plus Apache/OPcache headroom. Continue normal post-deploy OOM monitoring, but the old 768M action item is complete. |
| TD15 | Validate artisan-cache fail-fast behavior in prod | ⚠️ Partial | Low | `Dockerfile.prod` now runs `config:cache`, `route:cache`, and `event:cache` fail-fast at container startup. A documented fallback exists at `docker/entrypoint-cache.sh` to switch to soft runtime caching if production proves too fragile. Still needs observation during the next explicit production deploy; no deploy has been run for this validation. |
...
| AG72 | ⚠️ Tenant-branded native mobile app | Foundation 2026-04-29; follow-ups 2026-04-30 | Added the tenant-branded readiness layer to the existing native-app config: store mode (`shared` vs `tenant_branded`), build profile, iOS App Store ID, Android Play Store ID, marketing/privacy/support URLs, tenant push sender/channel routing fields, validation, and readiness booleans for iOS identity, Android identity, store metadata, push routing, and full tenant-branded readiness. The React admin screen now writes the real `native_app_*` backend keys and can export `/v2/admin/config/native-app/build-manifest` as the JSON handoff for a later white-label build pipeline. Follow-up hardening now reports exact missing tenant-branded requirements in both the settings response and build manifest, and the admin surface uses translation keys for its visible labels/toasts. This does not yet create separate signed App Store / Play Store builds or CI/CD per tenant. |
...
#### Demo Readiness Gap Analysis — AG74–AG77 (2026-04-30)

> Four items identified by cross-referencing the updated Agoris/KISS/NEXUS alignment document (2026-04-30) against the live codebase. The prior response pack referenced a "Warmth Pass" concept that had no implementation; a care-recipient-centred circle view and B2G cost-savings metrics were also identified as missing for the demo.

| # | Item | Priority | Notes |
|---|---|---|---|
| AG74 | ✅ Warmth Pass — community trust credential | Done 2026-04-30 | Auto-generated trust credential for members at Trusted tier (2) and above. `WarmthPassService::buildPass()` computes tier, hours_logged, reviews_received, identity_verified, caring categories contributed, member-since, and pass-active-since from existing data (no new table). `WarmthPassController`: `GET /v2/caring-community/my-warmth-pass` (member) + `GET /v2/admin/caring-community/warmth-pass/{userId}` (admin). Member-facing `WarmthPassPage` at `/caring-community/warmth-pass`: visual credential card with warm gradient, tier chip, stats, categories, attribution footer, "not eligible" state for tier < 2. Admin `WarmthPassAdminPage`: member-ID lookup tool to inspect any member's pass. All 11 language files updated. Wired into `App.tsx` and `admin/routes.tsx`. Closes the gap where the response pack mentioned "Warmth Pass" but no code existed. |
| AG75 | ✅ Care-recipient circle view | Done 2026-04-30 | Coordinator-facing view centred on a care recipient — shows their full circle of support rather than only the caregiver's perspective. `AdminCaringCommunityController::recipientCircle(int $userId)` at `GET /v2/admin/caring-community/recipient/{userId}/circle`. Returns recipient profile, all active support relationships with per-supporter hours and trust tier, total hours received, open help requests count, safeguarding flags count. Admin `CareRecipientCirclePage` at `/admin/caring-community/recipient-circle`: member-ID lookup → recipient profile card → summary stats (hours, supporters, open requests, safeguarding flag) → support relationships table with trust-tier chips. Addresses the missing "circle of support around one elderly person" that demo Journey 1 requires coordinators to see. |
| AG76 | ✅ Municipal ROI / B2G cost-savings dashboard | Done 2026-04-30 | Fills the "Pflege reduction metrics tailored to B2G procurement language" gap identified in the gap analysis. `AdminCaringCommunityController::municipalRoi()` at `GET /v2/admin/caring-community/municipal-roi`. Aggregates: total approved hours, active members (last 12 months), active caring relationships, distinct recipients, year-on-year hours trend. Applies CHF 35/hr (Swiss formal care assistant rate, SECO 2024) × 2× prevention multiplier (Age-Stiftung/KISS evaluation methodology). `MunicipalRoiAdminPage` at `/admin/caring-community/municipal-roi`: methodology note, 4-stat KPI row, large CHF offset number, prevention value, social isolation prevented, trend chip, and "What this means for municipalities" narrative with Age-Stiftung/Pro Senectute/cantonal reporting callout. Provides the punchline for demo Journey 3. |
| AG77 | ✅ Sub-regional geographic units | Done 2026-04-30 | Backend foundation: tenant-scoped `caring_sub_regions` for Quartier/Ortsteil/municipality/canton subdivisions, with active/inactive status, postal-code metadata, optional centroid/boundary payloads, member-readable active listing, admin CRUD APIs, and care-provider `sub_region_id` filtering. Frontend completion 2026-04-30: `SubRegionsAdminPage` (`/admin/caring-community/sub-regions`) with HeroUI table + create/edit modal, sidebar entry, route gated by `caring_community`; reusable `SubRegionFilter` component with localStorage cache and graceful soft-fail on tenants without sub-regions; wired into `CareProviderDirectoryPage` and `MarktPage`. Markt API now resolves `sub_region_id` to centre coordinates with type-derived radius (2km Quartier / 5km Ortsteil / 10km Gemeinde / 50km Kanton) so listings + marketplace items respect locality without schema changes. Translation keys `sub_region.*` + native German added; English fallbacks propagated to all 11 languages. Remaining: canton-by-canton address-data agreements before polygon-based routing (deferred to AG87 external integration backlog). |

#### Evaluation-to-Pilot Roadmap — AG78–AG88 (2026-04-30)

> These are the next practical steps after the demo-readiness build. They are deliberately framed as validation, governance, and pilot-readiness gates rather than another speculative feature sprint. The platform already has enough product for a KISS-only evaluation; the risk now is agreeing the operating model, legal/commercial boundary, and Swiss field fit.

| # | Item | Priority | Notes |
|---|---|---|---|
| AG78 | ✅ Guided evaluation session pack | Done 2026-04-30 (template) | `docs/CARING_COMMUNITY_PILOT_EVALUATION.md` is a 60–90 minute structured walkthrough with three paths (resident, coordinator, municipality) plus a Path D commercial-boundary review using AG82. Produces a four-bucket matrix ("NEXUS today / configure / build / keep partner-specific") and feeds into the AG88 decision memo. Live admin surfaces referenced: AG29/31/54/55/62/65/66/67/69/70/71/74/75/76/77/80/81/82/83/84/85/87. |
| AG79 | ⚠️ Swiss-native terminology and UX review | First-pass done 2026-04-30; native validation outstanding | **DE first-pass shipped (commit 640afd3ad).** Three changes across 19 DE namespace files: (1) global `ß → ss` Swiss spelling conversion across all DE files; (2) KISS-flavoured terminology pass on `caring_community.json` (Stellvertretung, Vertrauensausweis, Begleitete Person, Stellvertretende Begleitung); (3) translated all remaining English fallbacks in resident-facing files — `civic_digest.json` (AG90), `municipality_feedback.json` (AG92), `success_stories.json` (AG91), `onboarding.safeguarding.confirmation`, `profile.json` block/disconnect actions, `settings.appearance_prefs.accessibility_profile` + `push_campaign`, `common.json` request_help/coupon/regional_points/verein_import (~120 keys). Translation choices align with KISS Foundation public materials: Zeitvorsorge, Nachbarschaftshilfe, Caring Community, Tandem, Gemeinde, Kanton, Verein, Spitex, Angehörige, Pflege. **STILL OUTSTANDING:** Swiss-German native-speaker review of register (Sie/Du), KISS Foundation phrasing validation, FR/IT terminology pass (only DE was touched). The item stays open until a native validator signs off. |
| AG80 | ✅ FADP/nDSG disclosure and retention pack | Done 2026-04-30 (admin surface) | `PilotDisclosurePackService` stores an editable JSON envelope under the `caring.disclosure_pack` setting with sensible Swiss defaults (controller/processor matrix, sub-processors, data categories + lawful basis, retention defaults, data subject rights, federation policy, isolated-node configuration, incident response, cross-border transfer safeguards). `DisclosurePackAdminPage` (`/admin/caring-community/disclosure-pack`) edits each section via tabs and exports a Markdown disclosure document for legal review. Coexists with the AG42 machine-derived `/v2/admin/fadp/disclosure-pack` (consent ledger + processing register from runtime data) — this AG80 surface is the human-edited pilot legal-handover document. Formal legal certification still external. |
| AG81 | ✅ KISS operating-policy workshop | Done 2026-04-30 | `OperatingPolicyService` persists 11 discrete tenant settings under the `caring.operating_policy.*` key namespace: approval authority (admin/coordinator/mutual), trusted-reviewer threshold, SLA first-response and help-request windows, legacy-hour settlement policy, reciprocal-balance intervention threshold, safeguarding escalation owner, CHF hourly rate, CHF prevention multiplier, statement cadence (monthly/quarterly/annual), and signed policy appendix URL. Per-field schema-driven validation with min/max/enum guards. `OperatingPolicyAdminPage` (`/admin/caring-community/operating-policy`) groups fields into Approvals & Trust, SLA Windows, Hour Management, Safeguarding, CHF Methodology, and Pilot Policy Appendix sections. Settings are tenant-scoped and audit-logged via `tenant_settings.updated_at`. Workshop output captured in product; signed policy appendix link stored alongside. |
| AG82 | ✅ Commercial boundary map | Done 2026-04-30 | `CommercialBoundaryService` exposes a 31-capability matrix across 7 categories (core_caring, community_governance, gamification_engagement, commercial_layer, advanced_ai, mobile_native, regional_intelligence) classifying each as `agpl_public`, `tenant_config`, `private_deployment`, or `commercial`. Per-tenant overrides stored under `caring.commercial_boundary` setting key. `CommercialBoundaryAdminPage` (`/admin/caring-community/commercial-boundary`) shows capabilities grouped by category with classification chips, supports per-capability overrides, exports the matrix as Markdown for partner conversations. Defaults flag `paid_regional_analytics` + `partner_api_access` as commercial; `tenant_branded_native_app` as private_deployment; `local_advertising_campaigns` + `paid_push_campaigns` + `premium_member_tier` as tenant_config. |
| AG83 | ✅ Pilot success metrics and baseline capture | Done 2026-04-30 | `PilotScoreboardService` aggregates the 10 evaluation-roadmap metrics on a 90-day rolling window: active members, median first-response hours, approved hours, recurring relationships, coordinator workload, satisfaction (1–5 likert from municipality surveys), social isolation proxy %, communications reach %, business participation, cost offset CHF (hours × CHF 35 × 2 prevention multiplier). Baselines persist into `caring_kpi_baselines` with a `pilot_scoreboard` envelope distinguishing them from AG66 KPI baselines and a canonical `pre_pilot` label. Quarterly review cadence enforced via 3-month next-due timestamp with overdue chip. `PilotScoreboardAdminPage` (`/admin/caring-community/pilot-scoreboard`) shows pre-pilot vs current with per-metric deltas and percent change, captures pre-pilot baselines (one-time) and quarterly review snapshots, lists all captured baselines. Builds on (does not duplicate) AG66 and AG76 surfaces. |
| AG84 | ✅ Tenant data-quality and seed-to-real migration checklist | Done 2026-04-30 | `TenantDataQualityService` runs 10 read-only checks and reports each as ok/info/warning/danger with counts: duplicate emails (danger), duplicate phones (warning), missing preferred_language, missing sub_region, missing coordinator assignment, unverified organisations, seed-marker users (`@example.com` / `Test*` / `Demo*` — danger if any), unanswered help requests >30d, members without role, tenant-setting completeness (points to AG80/AG81). 6 checks support per-row drill-down. `DataQualityAdminPage` (`/admin/caring-community/data-quality`) shows summary chips + per-check cards + drill-down modals. Schema-driven simplifications documented inline where columns are missing on this schema (e.g. `users.sub_region_id`, `vol_organizations.verified_at` — checks return ok with a note rather than counting). |
| AG85 | ✅ Isolated-node decision gate | Done 2026-04-30 | `IsolatedNodeReadinessService` tracks 11 decisions as discrete `tenant_settings` rows under `caring.isolated_node.*`: deployment_mode (hosted_tenant/hosted_custom_domain/canton_isolated_node), hosting_owner, smtp_owner, storage_owner, backup_owner, update_cadence, source_release_workflow, telemetry_default, federation_key_exchange, dpo_appointed, incident_runbook_url. Each item carries value+owner+status+notes JSON; status enum `pending/in_progress/decided/blocked`. Gate `closed=true` only when every item is decided. `IsolatedNodeAdminPage` (`/admin/caring-community/isolated-node`) shows progress bar, gate-open/gate-closed banner, and per-item edit modal. Must be closed before any canton requiring data sovereignty goes live. |
| AG86 | ✅ Tenant-branded mobile build plan | Done 2026-04-30 (plan doc) | `docs/CARING_COMMUNITY_MOBILE_BUILD_PLAN.md` is the operational plan: 4 trigger conditions for starting, 18 decisions with named owners, 12-step build pipeline, ongoing maintenance commitments (Apple Developer $99/yr, push cert renewal, OS-version compat, Privacy Manifest updates), and a decision flowchart. AG72's existing `/v2/admin/config/native-app/build-manifest` is the JSON handoff this plan consumes. Default posture: PWA / shared app is sufficient for evaluation. Plan does NOT execute any build; it only enumerates ownership. |
| AG87 | ✅ External integration backlog | Done 2026-04-30 | `ExternalIntegrationBacklogService` stores items in a single `caring.external_integrations` JSON envelope. Each item: name, category (banking/payment/identity_verification/professional_care/municipal_data/postal/ahv/healthcare/other), owner_name, owner_email, status (proposed/scoping/blocked/sandbox/live/deprecated), interface_spec_url, dsa_status (not_required/drafting/in_review/signed), sandbox_url, notes. Stable IDs `intg_<hex>` server-generated. Seed defaults provide 6 well-known integrations (AHV, Spitex, cantonal master-data, PostFinance, Twint, postal-address). `ExternalIntegrationsAdminPage` (`/admin/caring-community/external-integrations`) supports CRUD with confirm-modal delete and seed-defaults button. |
| AG88 | ✅ Post-walkthrough decision memo template | Done 2026-04-30 (template) | `docs/CARING_COMMUNITY_DECISION_MEMO_TEMPLATE.md` is the one-page memo template captured at the end of every AG78 walkthrough session: go/no-go, pilot scope, configuration commitments captured live (AG80/AG81/AG82/AG83/AG85/AG87), build commitments with owners + dates, partner-specific items kept out of the AGPL repo, legal/commercial blockers, workstream owners, and signoff lines. Filed memos go in `docs/decisions/<YYYY-MM-DD-pilot-name>.md` and become the authoritative input for any follow-up build sprint. |

#### Website Completeness Roadmap — AG89–AG94 (added 2026-04-30 from live agoris.ch scrape)

> This section captures the remaining explicit public-website claims that are not already cleanly represented as standalone demo moments. It does not reopen completed modules; where an item builds on a completed module, the completed work is referenced in the appendix below and the new item is scoped as an incremental demo/completeness layer.
...
| Completed area | Keep completion record unchanged | New appendix item |
|---|---|---|
| AG14 municipal announcements and official posts | Completed as one-way official communication channel. | AG89 adds AI drafting/moderation before publishing; AG92 adds two-way feedback after publishing. |
| AG35 personalisation and AG57 push campaigns | Completed as personalisation preferences and paid/targeted push infrastructure. | AG90 adds a civic digest product surface that combines filtered civic, care, event, Verein, marketplace, and safety information. |
| AG61 KI-Agenten | Completed as autonomous proposal framework for matching, nudges, routing, and summaries. | AG89 adds a specific municipality communication/moderation agent and proposal queue. |
| AG62 municipality surveys | Completed as structured survey builder and analytics. | AG92 adds informal feedback/dialogue that is lower-friction than a survey. |
...
| Workstream | Gate before build or launch | Owner to confirm |
|---|---|---|
| KISS workflow | Operating policy approved; coordinator roles assigned; hour-review SLA agreed | Martin / KISS cooperative |
| Resident UX | Swiss-native terminology review complete; elderly-user friction list prioritised | Christopher |
| Commercial AGORIS layer | Public AGPL vs private/commercial boundary agreed | Roland / AGORIS |
| Technical architecture | Hosted vs isolated-node decision made; federation policy accepted | Tom |
| Legal/data protection | FADP/nDSG disclosure pack reviewed; controller/processor roles clear | AGORIS/KISS legal reviewer |
| Evidence/ROI | Baseline metrics captured before launch; methodology caveats approved | Municipality/foundation sponsor |
...
### 2026-04-30 AG72 Readiness Hardening

Tenant-branded mobile readiness now includes a machine-readable `missing_requirements` array in both `/v2/admin/config/native-app` and `/v2/admin/config/native-app/build-manifest`. The admin Native App screen renders those missing requirements as a translated checklist and no longer adds new hardcoded English labels/toasts on that surface. Focused checks: `docker exec nexus-php-app php vendor/bin/phpunit tests/Laravel/Feature/Controllers/AdminConfigControllerTest.php --filter native_app` and `npx eslint src/admin/modules/system/NativeApp.tsx`.
