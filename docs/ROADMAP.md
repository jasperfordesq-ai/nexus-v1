...
| # | Item | Priority | Notes |
|---|---|---|---|
| AG78 | Guided evaluation session pack | Critical | Prepare a 60-90 minute walkthrough agenda for Martin, Roland, Christopher, and Tom with three paths: resident/care-recipient journey, coordinator/KISS workflow, and municipality/foundation evidence. Output must be a decision log, product-fit notes, and a "NEXUS today / configure / build / keep AGORIS-specific" matrix. Use `docs/DEMO_SCRIPT.md`, `docs/AGORIS_MARTIN_RESPONSE_PACK.md`, and `docs/AGORIS_DILIGENCE_QUESTION_PACK.md` as the source set. |
...
| AG82 | AGORIS commercial boundary map | High | Classify every AGORIS-adjacent capability as public AGPL NEXUS, tenant configuration, private deployment layer, or future separate commercial agreement. Pay special attention to advertising, paid push, premium tiers, regional analytics, partner APIs, native mobile branding, KI-Agenten, and merchant loyalty. Do not build further proprietary AGORIS-specific surfaces until this boundary is explicit. |
...
| AG84 | Tenant data-quality and seed-to-real migration checklist | High | Replace demo-seed assumptions with real pilot data rules: member import consent, Verein import source, organisation verification, municipality domain verification, postcode/Quartier fields, language preference, coordinator assignment, duplicate detection, and rollback/export plan. No real KISS cooperative should launch from sample Cham/Zug data. |
| AG85 | Isolated-node decision gate | Medium | Decide whether the first Swiss pilot can run as a hosted NEXUS tenant, hosted custom-domain tenant, or canton-controlled isolated node. If isolated, define hosting owner, SMTP, storage, backups, update cadence, source-release workflow, telemetry default, and federation key exchange. This gate must be closed before any canton that requires data sovereignty goes live. |
| AG86 | Tenant-branded mobile build plan | Medium | Turn AG72's build manifest into an operational plan only if AGORIS needs a branded app for the pilot. Define Apple/Google developer-account owner, bundle/package naming, screenshots, app review responsibility, release cadence, push sender ownership, and emergency rollback. Until then, treat PWA/shared app as sufficient for evaluation. |
| AG87 | External integration backlog | Medium | Keep banking/payment/admin interfaces, POS bridge, Spitex/professional-care integrations, AHV official submission, and municipal master-data feeds in a partner-dependent backlog. Each item needs a named external owner, interface specification, data-sharing agreement, and test sandbox before implementation. |
| AG88 | Post-walkthrough decision memo | Critical | After the first AGORIS/KISS walkthrough, produce a one-page memo with: go/no-go recommendation, exact pilot scope, disabled modules, legal/commercial blockers, owner per workstream, target pilot date, and whether the next phase is KISS-only, AGORIS commercial, or both. This memo becomes the authoritative input for any follow-up build sprint. |
