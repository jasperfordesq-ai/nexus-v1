// Copyright © 2024–2026 Jasper Ford
// SPDX-License-Identifier: AGPL-3.0-or-later
// Author: Jasper Ford
// See NOTICE file for attribution and acknowledgements.

import {
  formatLegacyCouponMinorAmount,
  formatMarketplaceCurrency,
  formatMarketplaceMinorAmount,
} from './marketplaceCurrency';

describe('marketplace currency formatting', () => {
  it('keeps marketplace cash values in major units', () => {
    expect(formatMarketplaceCurrency(25.5, 'GBP')).toMatch(/25\.50/);
  });

  it('does not invent decimals for zero-decimal currencies', () => {
    const formatted = formatMarketplaceCurrency(2500, 'JPY');

    expect(formatted).toMatch(/2[,.]500/);
    expect(formatted).not.toMatch(/[,.]00(?:\D|$)/);
  });

  it('converts only legacy fixed-coupon minor units', () => {
    expect(formatLegacyCouponMinorAmount(500, 'GBP')).toMatch(/5\.00/);
  });

  it('uses the provider exponent for zero-decimal minor amounts', () => {
    const formatted = formatMarketplaceMinorAmount(2500, 'JPY');

    expect(formatted).toMatch(/2[,.]500/);
    expect(formatted).not.toMatch(/[,.]00(?:\D|$)/);
  });
});
