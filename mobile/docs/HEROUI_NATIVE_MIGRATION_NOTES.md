...
- Added a native group Files tab backed by `/v2/groups/{id}/files`, with HeroUI Native file cards plus download and admin delete actions for shared member resources.
- Added a native group Media tab backed by `/v2/groups/{id}/media`, with HeroUI Native media cards, image/video filters, photo/video uploads via Expo ImagePicker, URL open actions, and admin delete actions.
- Added a native group Q&A tab backed by `/v2/groups/{id}/questions`, with HeroUI Native question cards, member question composer, detail expansion, answer cards, and answer submission.
- Added native group Q&A voting and answer acceptance controls backed by `/v2/groups/{id}/qa/vote` and `/v2/groups/{id}/answers/{answerId}/accept`, with HeroUI Native vote buttons and group-admin/question-asker accept-answer actions.
- Added a native group Wiki tab backed by `/v2/groups/{id}/wiki`, with HeroUI Native page rows, page reading, member page creation, and page editing.
- Added native group Wiki revision history and group-admin delete actions backed by `/v2/groups/{id}/wiki/{pageId}/revisions` and `DELETE /v2/groups/{id}/wiki/{pageId}`.
- Added a native group Tasks tab backed by `/v2/groups/{id}/tasks` and `/v2/team-tasks/{id}`, with HeroUI Native task cards, stats chips, status filters, creation, inline status cycling, priority updates, and assignment updates.
- Added a native group Analytics tab backed by `/v2/groups/{id}/analytics`, `/analytics/retention`, and `/analytics/comparative`, with an admin-only HeroUI Native dashboard for overview metrics, activity breakdown, top contributors, content performance, retention cohorts, group comparison, and 7/30/90 day windows.
- Added a native goal template picker backed by `/v2/goals/templates`, `/v2/goals/templates/categories`, and `/v2/goals/from-template/{templateId}`, using HeroUI Native cards, category buttons, chips, and loading states.
- Added a native skills Discover tab backed by `/v2/skills/categories`, `/v2/skills/categories/{id}`, and `/v2/skills/members`, using HeroUI Native tabs, cards, chips, loading/empty states, and profile-routing member rows for category browsing and members-by-skill lookup.
