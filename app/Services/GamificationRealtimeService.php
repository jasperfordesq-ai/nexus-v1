 ...
 * No DB queries — pure Pusher broadcasting, so no legacy delegation needed.
 *
 * Uses App\Services\PusherService for broadcasting.
 */
class GamificationRealtimeService
{
     ...
     */
    public function broadcastDailyReward(int $userId, array $reward): bool
    {
        return PusherService::trigger(
            PusherService::getUserChannel($userId),
            'daily-reward',
     ...
     */
    public function broadcastStreakMilestone(int $userId, int $days, int $bonusXP): bool
    {
        return PusherService::trigger(
            PusherService::getUserChannel($userId),
            'streak-milestone',
     ...
     */
    public function broadcastRankChange(int $userId, int $oldRank, int $newRank, string $leaderboardType = 'xp'): bool
    {
     ...
     */
    public function broadcastProgressUpdate(int $userId, array $progress): bool
    {
        return PusherService::trigger(
            PusherService::getUserChannel($userId),
            'progress-update',
     ...
     */
    public function broadcastShopPurchase(int $userId, array $item): bool
    {
        return PusherService::trigger(
            PusherService::getUserChannel($userId),
            'shop-purchase',
     ...
     */
    public function broadcastToUsers(array $userIds, string $event, array $data): int
    {
        $success = 0;
