...
class CaringCommunityAlertService
{
    ...
    private function recipientsWithoutTandem(int $tenantId): ?array
    {
        ...
        return [
            ...
            'message' => 'These members received help in the last six months but have no active recurring tandem. Pair them with a steady supporter to lock in continuity.',
            'count' => $count,
    ...
    private function inactiveMembers(int $tenantId): ?array
    {
        ...
        return [
            'id' => 'inactive_members',
            'severity' => 'info',
            'title' => 'Active members gone quiet',
            'message' => 'Members who logged hours in the last six months but nothing in the last 30 days. A friendly check-in often re-engages them.',
            'count' => $count,
    ...
    private function overdueReviews(int $tenantId): ?array
    {
        ...
        return [
            'id' => 'overdue_reviews',
            'severity' => 'warning',
            'title' => 'Hour reviews past SLA',
            'message' => "Pending vol logs older than {$sla} days. Approve or escalate them so members get credited.",
            'count' => $count,
    ...
    private function coordinatorsOverloaded(int $tenantId): ?array
    {
        ...
        return [
            ...
            'severity' => 'critical',
            'title' => 'Coordinators overloaded',
            'message' => 'One or more coordinators have more than 10 pending reviews assigned. Reassign before bottlenecks form.',
            'count' => $count,
    ...
    private function overdueCheckIns(int $tenantId): ?array
    {
        ...
        return [
            ...
            'severity' => 'warning',
            'title' => 'Tandem check-ins overdue',
            'message' => 'Active recurring relationships whose scheduled check-in has passed. Reach out to confirm both sides are still well.',
            'count' => $count,
    ...
    private function lowSupply(int $tenantId): ?array
    {
        ...
        return [
            'id' => 'low_supply',
            'severity' => 'info',
            'title' => 'Categories with unmet demand',
            'message' => 'Categories where active requests outnumber available offers. Worth highlighting to potential supporters.',
            'count' => $count,
