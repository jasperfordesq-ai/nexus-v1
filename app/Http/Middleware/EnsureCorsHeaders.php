...
class EnsureCorsHeaders
{
    ...
    private function addCorsHeaders(Request $request, Response $response): Response
    {
        ...
        if (CorsHelper::isOriginAllowed($origin)) {
            ...
            $response->headers->set('Access-Control-Allow-Methods', 'GET, POST, PUT, PATCH, DELETE, OPTIONS');
            $response->headers->set('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-Requested-With, X-CSRF-TOKEN, Accept, X-Tenant-Id, X-Tenant-Slug, X-Trusted-Device, X-Timezone, X-Locale, X-Request-Id');
            $response->headers->set('Access-Control-Expose-Headers', 'X-Request-Id, X-Build');
        }
    ...
    {
        try {
            $count = \Illuminate\Support\Facades\DB::table('federation_external_partners')
                ->where('base_url', 'like', rtrim($origin, '/') . '%')
                ->where('status', 'active')
                ->count();
            return $count > 0;
        } catch (\Throwable) {
