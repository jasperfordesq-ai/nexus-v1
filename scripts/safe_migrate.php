...
try {
    ...
    try {
        $stmt = $pdo->prepare("
            INSERT INTO migrations (migration_name, backups, executed_at)
            VALUES (?, ?, NOW())
        ");
        $name = basename($migrations[0]);
        $stmt->execute([$name, $name]);
        success("Migration recorded in database");
    } catch (Exception $e) {
        warn("Could not record migration (migrations table might not exist): " . $e->getMessage());
    }
