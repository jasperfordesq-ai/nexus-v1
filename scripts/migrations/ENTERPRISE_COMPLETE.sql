-- ============================================
-- ENTERPRISE FEATURES - Complete Database Schema
-- Project NEXUS
-- Run this on BOTH local and live servers
-- ============================================

-- ============================================
-- SECTION 1: GDPR COMPLIANCE TABLES
-- ============================================
...
-- Data Processing Activity Log (Article 30 ROPA)
...
-- Data Breach Log (Articles 33-34)
...
-- ============================================
-- SECTION 2: USER TABLE MODIFICATIONS
-- Add GDPR-related columns to users table
-- ============================================

-- Run these ALTER statements to add GDPR columns to users table
-- These use IF NOT EXISTS pattern for safety

SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'users' AND COLUMN_NAME = 'anonymized_at') = 0,
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'users' AND COLUMN_NAME = 'gdpr_export_requested_at') = 0,
    'ALTER TABLE users ADD COLUMN gdpr_export_requested_at DATETIME NULL',
    'SELECT 1'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
     WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'users' AND COLUMN_NAME = 'gdpr_deletion_requested_at') = 0,
    'ALTER TABLE users ADD COLUMN gdpr_deletion_requested_at DATETIME NULL',
    'SELECT 1'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
...
-- ============================================
-- SECTION 3: DEFAULT DATA RETENTION POLICIES
-- ============================================

INSERT INTO data_retention_policies (tenant_id, data_category, table_name, retention_days, deletion_method, legal_basis) VALUES
(1, 'session_data', 'sessions', 30, 'hard_delete', 'No longer needed for service provision'),
(1, 'login_history', 'login_history', 365, 'anonymize', 'Security audit requirement - 1 year'),
(1, 'activity_logs', 'activity_log', 730, 'anonymize', 'Operational analytics - 2 years'),
(1, 'notifications', 'notifications', 90, 'hard_delete', 'No longer relevant after 90 days'),
(1, 'messages', 'messages', 1095, 'soft_delete', 'Communication records - 3 years'),
(1, 'gdpr_audit_log', 'gdpr_audit_log', 2555, 'hard_delete', 'GDPR compliance records - 7 years'),
(1, 'deleted_users', 'users', 30, 'hard_delete', 'Grace period for account recovery')
ON DUPLICATE KEY UPDATE updated_at = CURRENT_TIMESTAMP;

-- ============================================
-- SECTION 4: DEFAULT DATA PROCESSING ACTIVITIES (Article 30 ROPA)
-- ============================================

INSERT INTO data_processing_log (tenant_id, processing_activity, purpose, legal_basis, data_categories, data_subjects, recipients, retention_period, dpia_required) VALUES
(1, 'User Registration', 'Create and manage user accounts', 'contract', '["name", "email", "password_hash", "profile_data"]', '["platform_users"]', '["internal_staff"]', 'Duration of account + 30 days', FALSE),
(1, 'Time Bank Transactions', 'Record and manage time exchange transactions', 'contract', '["transaction_details", "user_ids", "hours", "descriptions"]', '["platform_users"]', '["internal_staff", "other_users"]', '7 years (financial records)', FALSE),
(1, 'Email Communications', 'Send transactional and marketing emails', 'consent', '["email", "name", "preferences"]', '["platform_users", "newsletter_subscribers"]', '["email_service_provider"]', 'Until consent withdrawn', FALSE),
(1, 'Analytics', 'Understand platform usage and improve services', 'legitimate_interests', '["anonymized_usage_data", "page_views", "feature_usage"]', '["platform_users"]', '["analytics_provider"]', '2 years', FALSE),
(1, 'Support Tickets', 'Provide customer support', 'contract', '["name", "email", "support_communications"]', '["platform_users"]', '["internal_staff"]', '3 years', FALSE),
(1, 'Security Monitoring', 'Detect and prevent security threats', 'legitimate_interests', '["ip_addresses", "login_attempts", "user_agents"]', '["platform_users", "visitors"]', '["internal_staff", "security_tools"]', '1 year', FALSE)
ON DUPLICATE KEY UPDATE updated_at = CURRENT_TIMESTAMP;

-- ============================================
-- SECTION 5: VERIFICATION (Optional - Run Separately)
-- ============================================
-- To verify installation, run these queries AFTER the migration completes:
--
-- SELECT 'gdpr_requests' AS tbl, COUNT(*) AS exists_check FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'gdpr_requests';
-- SELECT slug, name, is_required FROM consent_types ORDER BY display_order;
-- SELECT data_category, retention_days FROM data_retention_policies;
--
-- ============================================
-- DONE!
-- All enterprise tables have been created.
