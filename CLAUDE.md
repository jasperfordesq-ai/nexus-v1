...
This applies to ALL agents, including background agents and teammate agents in swarm mode. No agent may initiate SSH, run `bluegreen-deploy.sh` / `safe-deploy.sh`, or trigger any production deployment autonomously.
...
- The complete shared component library is the official `govuk-frontend` package; see `accessible-frontend/COMPONENTS.md` for the imported component inventory and allowed/restricted component list.
...
- GOV.UK Frontend code/sample code is MIT; GOV.UK documentation is Crown copyright under OGL v3.0. If copied or closely adapted, record attribution in `docs/govuk-alpha/ATTRIBUTION.md`.
...
1. Re-deploy: `sudo bash scripts/deploy/bluegreen-deploy.sh deploy --detach`
2. Rollback: `sudo bash scripts/deploy/bluegreen-deploy.sh rollback --detach`
...
When the user says **"deploy"** (or any variation), **ALWAYS use `bluegreen-deploy.sh deploy --detach`**:

```bash
# Launch deploy with the canonical blue-green engine
ssh -i "C:\ssh-keys\project-nexus.pem" -o RequestTTY=force azureuser@20.224.171.253 \
  "cd /opt/nexus-php && sudo bash scripts/deploy/bluegreen-deploy.sh deploy --detach"
...
**How it works on production:** `bluegreen-deploy.sh deploy --detach` builds the inactive color in the background while live traffic continues uninterrupted. There is no maintenance window.

**NEVER run without `--detach`** — Docker image builds take 10+ minutes and will timeout an SSH session without it.

**NEVER manually choose `full` or `quick`** — those modes only apply to the legacy maintenance-mode fallback when blue-green is not configured.
