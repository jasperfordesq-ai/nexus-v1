...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::table('federation_messages', function (Blueprint $table): void {
            if (!Schema::hasColumn('federation_messages', 'notification_sent_at')) {
                $table->timestamp('notification_sent_at')->nullable()->after('external_message_id');
            }
            if (!Schema::hasColumn('federation_messages', 'email_sent_at')) {
                $table->timestamp('email_sent_at')->nullable()->after('notification_sent_at');
            }
            if (!Schema::hasColumn('federation_messages', 'email_failed_at')) {
                $table->timestamp('email_failed_at')->nullable()->after('email_sent_at');
            }
            if (!Schema::hasColumn('federation_messages', 'email_last_error')) {
    ...
    public function down(): void
    {
        ...
        Schema::table('federation_messages', function (Blueprint $table): void {
            foreach (['email_last_error', 'email_failed_at', 'email_sent_at', 'notification_sent_at'] as $column) {
                if (Schema::hasColumn('federation_messages', $column)) {
